...
public abstract class ListMessage extends Message {
	...
	public ListMessage(NetworkParams params, byte[] bytes) throws ProtocolException {
		super(params, bytes, 0);
	}

	public ListMessage(NetworkParams params, byte[] payload, boolean parseLazy, boolean parseRetain, int length)
			throws ProtocolException {
		super(params, payload, 0, parseLazy, parseRetain, length);
	}

	public ListMessage(NetworkParams params) {
		super(params);
	...
	public void addItem(InventoryItem item) {
		unCache();
		length -= VariableInt.sizeOf(items.size());
		items.add(item);
		length += VariableInt.sizeOf(items.size()) + InventoryItem.MESSAGE_LENGTH;
	}
	...
	public void bitcoinSerializeToStream(OutputStream stream) throws IOException {
		stream.write(new VariableInt(items.size()).encode());
