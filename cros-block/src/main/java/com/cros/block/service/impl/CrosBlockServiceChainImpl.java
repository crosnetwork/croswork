...
import com.cros.block.service.ICrosBlockChainService;
...
@Service("crosBlockChainService")
public class CrosBlockServiceChainImpl implements ICrosBlockChainService {
	@Resource(name = "mongoDBBaseDao")
	MongoDBBaseDao mongoDBBaseDao;
	private Log logger = LogFactory.getLog(CrosBlockServiceChainImpl.class);
	
	@Override
	public void saveToblockChain(String blockInfoId) {
		/*
		 *  业务逻辑
		...
		DBCursor dbs_pending = mongoDBBaseDao.queryForTxHash("TxPool"+blockInfoId);
		...
		while (dbs_pending.hasNext()) {
			...
			if(!verifyTx(new Document(jsonObject))){
				...
				values.add(id);
				mongoDBBaseDao.updateTxStatusInvalid("TxPool"+blockInfoId, values);
				continue;
        	}
			...
			if(Tx_hashs.size()==100){
				...
				createBlockchain(jsonArray,ids,Tx_hashs,blockInfoId);
				// 清理 ids 和Tx_hashs
				ids.clear();
				Tx_hashs.clear();
				jsonArray.clear();
	...
	private void createBlockchain(JSONArray jsonArray,List<ObjectId> ids,List<String> Tx_hashs,String blockInfoId){
		...
		// 
		DBCursor prev_block_Cur = mongoDBBaseDao.queryForPrevBlock("Blockchain"+blockInfoId);
		try{
...
//        }
		// 校验
//		if(flag){
			mongoDBBaseDao.add(doc, "Blockchain"+blockInfoId);
			//更新txpool中的状态 
			BasicDBList values = new BasicDBList();
			for(ObjectId id:ids){
				values.add(id);
			}
			mongoDBBaseDao.updateTxStatusConfirmed("TxPool"+blockInfoId, values);
//		}
	}
