...
public class CrosPeerDiscovery implements NetworkPeerDiscovery {
	private static final Logger log = LoggerFactory.getLogger(CrosPeerDiscovery.class);
	...
	public static final int RESOLVE_STREAM_ID = 0x1000;
	...
	private final NetworkParams netParams;
	private final CircuitPathChooser pathChooser;
	private final TorClient torClient;
	private ListeningExecutorService threadPool;

	public CrosPeerDiscovery(NetworkParams netParams, TorClient torClient) {
		this(netParams.getDnsSeeds(), netParams, torClient);
	}

	public CrosPeerDiscovery(String[] hostNames, NetworkParams netParams, TorClient torClient) {
	...
	public InetSocketAddress[] getPeers(long timeoutValue, TimeUnit timeoutUnit) throws NetworkDiscoveryException {
		if (hostNames == null)
			throw new NetworkDiscoveryException("Unable to find any peers via DNS");
		...
		try {
			List<Circuit> circuits = getCircuits(torClient.getConfig().getCircuitBuildTimeout(), TimeUnit.MILLISECONDS,
					routers);
			if (circuits.isEmpty())
				throw new NetworkDiscoveryException(
						"Failed to open any circuit within " + String.valueOf(timeoutValue) + " " + timeoutUnit);
			...
			if (addresses.size() < MINIMUM_ROUTER_COUNT)
				throw new NetworkDiscoveryException("Unable to find enough peers via Tor - got " + addresses.size());
	...
	private List<Circuit> getCircuits(long timeoutValue, TimeUnit timeoutUnit, Set<Router> routers)
		...
		checkArgument(routers.size() >= MINIMUM_ROUTER_LOOKUP_COUNT,
				"Set of {} routers is smaller than required minimum {}", routers.size(), MINIMUM_ROUTER_LOOKUP_COUNT);
	...
	private synchronized void createThreadPool(int size) {
		threadPool = MoreExecutors
				.listeningDecorator(Executors.newFixedThreadPool(size, new ContextThreadFactory("Tor DNS discovery")));
	}

	private InetAddress lookup(Circuit circuit, String seed) throws UnknownHostException {

		RelayCell cell = circuit.createRelayCell(RelayCell.RELAY_RESOLVE, RESOLVE_STREAM_ID,
