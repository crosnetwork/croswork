...
public class BIP38 extends Checksum {

	public transient NetworkParams params;
	...
	public BIP38(NetworkParams params, String encoded) throws NoAddressException {
		super(encoded);
		this.params = checkNotNull(params);
		if (version != 0x01)
			throw new NoAddressException("Mismatched version number: " + version);
		if (bytes.length != 38)
			throw new NoAddressException("Wrong number of bytes, excluding version byte: " + bytes.length);
		hasLotAndSequence = (bytes[1] & 0x04) != 0;
		compressed = (bytes[1] & 0x20) != 0;
		if ((bytes[1] & 0x01) != 0)
			throw new NoAddressException("Bit 0x01 reserved for future use.");
		if ((bytes[1] & 0x02) != 0)
			throw new NoAddressException("Bit 0x02 reserved for future use.");
		if ((bytes[1] & 0x08) != 0)
			throw new NoAddressException("Bit 0x08 reserved for future use.");
		if ((bytes[1] & 0x10) != 0)
			throw new NoAddressException("Bit 0x10 reserved for future use.");
		final int byte0 = bytes[0] & 0xff;
		if (byte0 == 0x42) {

			if ((bytes[1] & 0xc0) != 0xc0)
				throw new NoAddressException("Bits 0x40 and 0x80 must be set for non-EC-multiplied keys.");
			ecMultiply = false;
			if (hasLotAndSequence)
				throw new NoAddressException("Non-EC-multiplied keys cannot have lot/sequence.");
		} else if (byte0 == 0x43) {

			if ((bytes[1] & 0xc0) != 0x00)
				throw new NoAddressException("Bits 0x40 and 0x80 must be cleared for EC-multiplied keys.");
			ecMultiply = true;
		} else {
			throw new NoAddressException("Second byte must by 0x42 or 0x43.");
		}
	...
	private ECKey decryptEC(String normalizedPassphrase) {
		try {
			...
			byte[] passFactorBytes = SCrypt.scrypt(normalizedPassphrase.getBytes(Charsets.UTF_8), ownerSalt, 16384, 8,
					8, 32);
			if (hasLotAndSequence) {
	...
	private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
		...
		params = checkNotNull(NetworkParams.fromID(in.readUTF()));
	}
}
