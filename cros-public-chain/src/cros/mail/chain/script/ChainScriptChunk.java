...
public class ChainScriptChunk {
	public final int opcode;
	@Nullable
	public final byte[] data;
	private int startLocationInProgram;

	public ChainScriptChunk(int opcode, byte[] data) {
		this(opcode, data, -1);
	}

	public ChainScriptChunk(int opcode, byte[] data, int startLocationInProgram) {
		this.opcode = opcode;
		this.data = data;
	...
	public int decodeOpN() {
		checkState(isOpCode());
		return ChainScript.decodeFromOpN(opcode);
	}
	...
	public void write(OutputStream stream) throws IOException {
		if (isOpCode()) {
			...
			if (opcode < OP_PUSHDATA1) {
				...
				checkState(data.length <= ChainScript.MAX_SCRIPT_ELEMENT_SIZE);
	...
	public String toString() {
		...
		if (isOpCode()) {
			...
			buf.append(ChainScript.decodeFromOpN(opcode));
		}
		return buf.toString();
	}
	...
	public boolean equals(Object o) {
		...
		ChainScriptChunk other = (ChainScriptChunk) o;
